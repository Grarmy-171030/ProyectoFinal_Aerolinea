...
class VentanaAerolinea(Frame):
    ...
    def listar_pasajeros(self):
        ...
        self.text_resultados.config(state=NORMAL)
        self.text_resultados.delete(1.0, END)
        self.text_resultados.insert(END, f"Lista de pasajeros - Vuelo {vuelo.origen} → {vuelo.destino}\n\n")

        # Pasajeros en clase Business
        self.text_resultados.insert(END, "Clase Business ($500):\n")
        encontrados = False
        ...
        for fila in range(1, filas_business + 1):
            for butaca in range(1, butacas + 1):
                ...
                if pasajero:
                    ...
                    letra = chr(64 + butaca)
                    self.text_resultados.insert(END,
                                                f"Fila {fila}{letra}: {pasajero.get_nombre()} ({pasajero.get_edad()} años)\n")

        if not encontrados:
            self.text_resultados.insert(END, "No hay pasajeros en esta clase\n")

        # Pasajeros en clase Turista
        self.text_resultados.insert(END, "\nClase Turista ($200):\n")
        encontrados = False
    ...
    def mostrar_menores(self):
        ...
        self.text_resultados.config(state=NORMAL)
        self.text_resultados.delete(1.0, END)
        self.text_resultados.insert(END, f"Pasajeros menores de 15 años - Vuelo {vuelo.origen} → {vuelo.destino}\n\n")

        # Menores en clase Business
        self.text_resultados.insert(END, "Clase Business ($500):\n")
        encontrados = False
        ...
        if not encontrados:
            self.text_resultados.insert(END, "No hay menores en esta clase\n")

        # Menores en clase Turista
        self.text_resultados.insert(END, "\nClase Turista ($200):\n")
        encontrados = False
    ...
    def mostrar_ingresos(self):
        ...
        self.text_resultados.config(state=NORMAL)
        self.text_resultados.delete(1.0, END)
        self.text_resultados.insert(END, "=== REPORTE DE INGRESOS ===\n\n")

        total_general = 0

        for vuelo in self.aerolinea.vuelos:
            ingresos = vuelo.calcular_ingresos()
            ingresos_business = ingresos[0] * vuelo.precio_business
            ingresos_turista = ingresos[1] * vuelo.precio_turista
            total_vuelo = ingresos_business + ingresos_turista
            total_general += total_vuelo

            self.text_resultados.insert(END,
                                        f"Vuelo {vuelo.origen} → {vuelo.destino}:\n")
            self.text_resultados.insert(END,
                                        f"  Business ($500): {ingresos[0]} pasajeros → ${ingresos_business}\n")
            self.text_resultados.insert(END,
                                        f"  Turista ($200): {ingresos[1]} pasajeros → ${ingresos_turista}\n")
            self.text_resultados.insert(END,
                                        f"  TOTAL: ${total_vuelo}\n\n")

        self.text_resultados.insert(END, "=" * 50 + "\n")
        self.text_resultados.insert(END, f"INGRESOS TOTALES: ${total_general}\n")
        self.text_resultados.insert(END, "=" * 50 + "\n")
        self.text_resultados.config(state=DISABLED)
